  ...
  ! 
  !-----------------------------------------------------------------------
  SUBROUTINE print_mob( F_SERTA, F_SERTAcb, BZtoIBZ, s_BZtoIBZ, vkk_all, etf_all, wkf_all, ef0, efcb  ) 
  !-----------------------------------------------------------------------
  !!
  ...
  REAL(kind=DP), INTENT(in) :: F_SERTA(3, ibndmax-ibndmin+1, nstemp, nkqtotf/2)  
  !! SERTA solution 
  REAL(kind=DP), INTENT(in) :: F_SERTAcb(3, ibndmax-ibndmin+1, nstemp, nkqtotf/2)  
  !! SERTA solution 
      ...
      DO ik=1,  nkf
        DO ibnd = 1, ibndmax-ibndmin+1
          IF (etf_all (ibnd, ik + lower_bnd -1) > efcb(itemp) ) THEN
            tdf_sigma(:) = zero
            vk_cart(:) = vkk_all(:,ibnd, ik+lower_bnd -1)
            Fi_cart(:) = F_SERTAcb(:,ibnd,itemp, ik+lower_bnd -1)
            ! Loop on full BZ
            nb = 0
            DO ikbz=1, nkf1*nkf2*nkf3
              IF (BZtoIBZ(ikbz) == ik+lower_bnd -1) THEN
